...
| **BTC vanity address generator** | Developed algorithms to speeding up the cost and memory for generating vanity addresses using the optimization
techniques of Elliptic curve operation. Implemented process improvements thatincreased the efficiency of address generation by reducing computation time by up to 30%. Genterated 10 million addresses per seconds on modern CPU architectures / mulitple billions of addresses per second using GPU (CUDA) acceleration |
